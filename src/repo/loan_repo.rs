...
pub async fn save_new_loan(connection: &PgPool, loan: SaleLoan) -> AppResult<()> {
    let result = sqlx::query(INSERT_NEW_LOAN)
        .bind(loan.sale_id)
...
pub async fn get_loans_by_debtor_name(
    ...
    let result = sqlx::query_as::<_, LoanItem>(GET_LOAN_LIST)
        .bind(&name_debtor)
...
pub async fn get_payments_loan(connection: &PgPool, loan_id: Uuid) -> AppResult<Vec<LoanPayment>> {
    let result = sqlx::query_as::<_, LoanPayment>(GET_PAYMENTS_LOAN)
        .bind(loan_id)
        .fetch_all(connection)
        .await;
...
pub async fn add_new_payment_loan(
    ...
    if let Err(err) = sqlx::query(INSERT_NEW_PAYMENT_LOAN)
        .bind(loan)
        .bind(payment)
        .execute(connection)
        .await
...
pub async fn get_total_loans(
    ...
    let result = sqlx::query_as::<_, TotalLoans>(GET_LOAN_TOTAL)
        .bind(start_date)
        .bind(end_date)
