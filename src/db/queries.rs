...
/// Retrieve the current amount of each product in the catalog
///
/// # Columns
/// | column | description |
/// |---------|---------|
...
/// | `unit_measurement` | unit measurement of `amount` |
pub const GET_CATALOG_PRODUCTS: &str = r#"
select
    product.full_name as product_name,
    coalesce(sum("catalog".current_amount), 0) as amount,
    lower(unit_measurement.description) as unit_measurement
from product
...
where ($1 = '' or lower(product.full_name) like concat('%',$1,'%'))
group by
    product.full_name,
    product.min_amount,
...
where
	(
		$1 = ''
		or lower(loan.name_debtor) like concat('%', $1,'%')
	)
